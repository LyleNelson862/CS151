...
void Character::moveJones(sf::RenderWindow &window, sf::Event &event, int gridSize, int &stepcount)
{
    ...
    if (event.type == sf::Event::KeyPressed)
    {
        if (event.key.code == sf::Keyboard::Right)
        {
            if (xPos < 480)
            {
                /*  This function was not working because I couldn't get the origin to stay in the right spot

                if (facing == 'L')
                {
                    // mCharacter.setOrigin(sf::Vector2f(mCharacter.getTexture()->getSize().x * 0.25, 0));
                    mCharacter.setOrigin(sf::Vector2f(xPos, yPos));
        ...
        else if (event.key.code == sf::Keyboard::Left)
        {
            ...
            if (xPos > 0)
            {
                ...
                if (facing == 'R')
                {
                    ...
                    mCharacter.scale(-1.f, 1.f);
                    mCharacter.setOrigin(sf::Vector2f(xPos, yPos)); // This is a built-in function that flips the image.
                    facing = 'L';
                }
                */
                //else
